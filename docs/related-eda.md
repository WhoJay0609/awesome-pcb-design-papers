...
16 papers are assigned to this primary topic. Detailed fields come from `data/papers.json`; `not_reported` means that the accessible evidence does not state the field.
...
- **Dataset or data source:** `open`: ChatEDA-Bench
...
- **Dataset or data source:** `open_with_upstream_terms`: CircuitNet 2.0
